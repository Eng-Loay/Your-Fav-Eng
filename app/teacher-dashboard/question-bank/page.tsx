...
export default function TeacherQuestionBankPage() {
  ...
  const { data, loading, refetch } = useApi(() => api.getInstructorQuestionBank())
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await api.createInstructorQuestionBankItem({
      ...
      setShowAdd(false)
      setForm({ question: "", questionAr: "", type: "multiple_choice", options: [{ id: "a", text: "", textAr: "", isCorrect: false }], correctAnswerText: "" })
      refetch()
    } catch {
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id)
    try {
      await api.deleteInstructorQuestionBankItem(id)
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden">
          <div className="divide-y divide-[#E2E8F0]">
            {items.map((q) => {
              ...
              return (
                <div key={q.id} className="flex items-center justify-between p-4 hover:bg-[#F8FAFC] transition-colors">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#0F172A]">{isAr ? (q.questionAr || q.question) : q.question}</p>
                    {opts.length > 0 && (
                  ...
                  <button onClick={() => handleDelete(q.id)} disabled={deletingId === q.id} className="p-2 rounded-lg hover:bg-red-50 text-red-500 disabled:opacity-50">
          ...
          <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="space-y-4">
                    ...
                    <button
                      ...
                      onClick={() =>
                        setForm((f) => ({
                          ...f,
                          type: t.value,
                          options: t.value === "true_false"
                            ? [{ id: "true", text: "True", textAr: "صح", isCorrect: true }, { id: "false", text: "False", textAr: "خطأ", isCorrect: false }]
                            : [{ id: "a", text: "", textAr: "", isCorrect: false }],
                        }))
                      }
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 text-xs font-medium transition-all ${form.type === t.value ? "border-[#7C3AED] bg-[#7C3AED]/10 text-[#7C3AED]" : "border-[#E2E8F0] hover:border-[#94A3B8]"}`}
                    >
              ...
              <div>
                ...
                <input value={form.questionAr} onChange={(e) => setForm((f) => ({ ...f, questionAr: e.target.value }))} className="w-full border rounded-xl px-3 py-2 text-sm" placeholder={isAr ? "السؤال بالعربية" : "Question in Arabic"} />
              </div>
              <div>
                <label className="text-sm font-medium block mb-1">{isAr ? "السؤال (إنجليزي)" : "Question (En)"}</label>
                <input value={form.question} onChange={(e) => setForm((f) => ({ ...f, question: e.target.value }))} className="w-full border rounded-xl px-3 py-2 text-sm" placeholder={isAr ? "السؤال بالإنجليزية" : "Question in English"} />
              </div>
              {form.type === "short_answer" ? (
                <div>
                  <label className="text-sm font-medium block mb-1">{isAr ? "الإجابة الصحيحة" : "Correct Answer"}</label>
                  <input value={form.correctAnswerText} onChange={(e) => setForm((f) => ({ ...f, correctAnswerText: e.target.value }))} className="w-full border rounded-xl px-3 py-2 text-sm" placeholder={isAr ? "أدخل الإجابة الصحيحة" : "Enter correct answer"} />
                </div>
              ) : (
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    {form.type === "multiple_choice" && (
                      <Button type="button" variant="outline" size="sm" onClick={addOption} className="rounded-lg"><Plus className="w-3 h-3" /></Button>
                    )}
                  </div>
                  {form.options.map((opt, i) => (
                    <div key={opt.id} className="flex gap-2 mb-2 items-center">
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <input type="radio" name="correct" checked={opt.isCorrect} onChange={() => setForm((f) => ({ ...f, options: f.options.map((o, j) => ({ ...o, isCorrect: j === i })) }))} />
                        <span className="text-sm font-medium">{opt.textAr || opt.text || opt.id}</span>
                      </label>
                      {form.type !== "true_false" && (
                        <>
                          <input value={opt.textAr} onChange={(e) => setForm((f) => ({ ...f, options: f.options.map((o, j) => (j === i ? { ...o, textAr: e.target.value } : o)) }))} className="flex-1 border rounded-lg px-2 py-1 text-sm" placeholder="Ar" />
                          <input value={opt.text} onChange={(e) => setForm((f) => ({ ...f, options: f.options.map((o, j) => (j === i ? { ...o, text: e.target.value } : o)) }))} className="flex-1 border rounded-lg px-2 py-1 text-sm" placeholder="En" />
                        </>
                      )}
                    </div>
                  ))}
