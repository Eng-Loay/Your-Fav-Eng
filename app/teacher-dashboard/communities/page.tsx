...
export default function TeacherCommunitiesPage() {
  ...
  const { data: studentsRes } = useApi(() => api.getEnrolledStudents(), { deps: [] })

  const communities = (communitiesRes as { data?: any[] })?.data ?? []
  const students = (studentsRes as { data?: any[] })?.data ?? []
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="max-h-48 overflow-y-auto space-y-2 border border-[#E2E8F0]/60 rounded-xl p-2">
                {students.length === 0 ? (
                  <p className="text-sm text-[#94A3B8] p-2">{locale === "ar" ? "لا يوجد طلاب مسجلين" : "No enrolled students"}</p>
                ) : (
                  students.map((s: any) => (
                    <label
                      key={s.id}
