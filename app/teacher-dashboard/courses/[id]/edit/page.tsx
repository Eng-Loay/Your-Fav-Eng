...
export default function TeacherEditCoursePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.request(`/courses/${courseId}`, { method: "PUT", body })
  ...
  if (!course) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] gap-3">
        ...
        <Link href="/teacher-dashboard/courses" className="text-[#8B5CF6] hover:underline">
          العودة للدورات
        </Link>
      </div>
    )
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Link href={`/teacher-dashboard/courses/${courseId}/content`}>
            <Button variant="outline" className="gap-2 rounded-xl h-10 border-[#8B5CF6]/30 text-[#8B5CF6] hover:bg-[#8B5CF6]/5">
      ...
      >
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { value: "published", label: "إرسال للمراجعة", desc: "ستحتاج موافقة الأدمن للنشر" },
              ].map((s) => (
                <button
                  key={s.value}
                  type="button"
                ...
                >
                  <p className="text-sm font-bold text-slate-900">{s.value === "published" ? "إرسال للمراجعة" : s.label}</p>
          ...
          <div className="space-y-3">
            {[
              { key: "certificateEnabled" as const, label: "تفعيل الشهادة", desc: "منح شهادة إتمام", checked: form.certificateEnabled },
        ...
        <div className="flex items-center justify-between bg-white rounded-2xl border border-slate-200/60 shadow-sm p-5">
          ...
          <Button
            onClick={handleSave}
            disabled={saving}
            className="gap-2 rounded-xl bg-[#8B5CF6] hover:bg-[#7C3AED] text-white h-10 px-8"
          >
