...
interface Lesson {
  ...
  title: string
  description: string
  type: "video" | "text" | "quiz" | "pdf" | "game" | "assignment"
  videoSource: string
...
export default function TeacherContentBuilderPage() {
  ...
  const { data: contentBankData, refetch: refetchContentBank } = useApi(() => api.getInstructorContentBank(), { immediate: false })
  const { data: questionBankData, refetch: refetchQuestionBank } = useApi(() => api.getInstructorQuestionBank(), { immediate: false })
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      const res = await api.getInstructorCourseChapters(courseId)
  ...
  const openContentBank = (sectionId: string, subsectionId?: string) => {
  ...
  const addFromContentBank = (item: { id: string; title: string; titleAr?: string; type?: string; content?: string; videoUrl?: string; pdfUrl?: string; duration?: number }) => {
    if (!bankTargetSectionId) return
    const typeMap: Record<string, Lesson["type"]> = {
      VIDEO: "video",
      TEXT: "text",
      PDF: "pdf",
    }
    const lessonType = typeMap[item.type || "VIDEO"] || "video"
    const newLesson: Lesson = {
      ...emptyLesson(),
      type: lessonType,
      title: item.titleAr || item.title,
      duration: item.duration ?? 0,
      videoSource: item.type === "PDF" ? (item.pdfUrl ?? "") : (item.videoUrl ?? ""),
      content: item.content ?? "",
    }
    mutateSectionLessons(bankTargetSectionId, bankTargetSubsectionId ?? undefined, (lessons) => [...lessons, newLesson])
    setShowContentBank(false)
    setBankTargetSectionId(null)
    setBankTargetSubsectionId(null)
    showToast("تم إضافة المحتوى من البنك")
  }
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      const res = await api.syncInstructorCourseContent(courseId, chapters)
  ...
  const lessonTypeConfig: Record<string, { label: string; icon: React.ElementType; color: string }> = {
    video: { label: "فيديو", icon: PlayCircle, color: "text-blue-500 bg-blue-50" },
      ...
      >
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {lesson.type === "game" && isSavedId(lesson.id) && (
            <Link
              href={`/teacher-dashboard/games/${lesson.id}/host`}
          ...
          {lesson.type === "assignment" && isSavedId(lesson.id) && (
            <Link
              href={`/teacher-dashboard/course-assignments/${lesson.id}`}
  ...
  const renderAddLessonButtons = (sectionId: string, subsectionId?: string) => (
    <div className="px-4 py-3 border-t border-slate-100 flex flex-wrap gap-2">
      ...
      <Button
        variant="outline"
        size="sm"
        onClick={() => openContentBank(sectionId, subsectionId)}
        className="gap-1.5 rounded-lg text-xs h-8 border-amber-200 text-amber-700 hover:bg-amber-50"
      >
        <Library className="w-3.5 h-3.5 text-amber-500" />
        استورد من بنك المحتوى
      </Button>
      <Button
        variant="outline"
        size="sm"
        onClick={() => openQuestionBank(sectionId, subsectionId)}
        className="gap-1.5 rounded-lg text-xs h-8 border-violet-200 text-violet-700 hover:bg-violet-50"
      >
        <BookOpen className="w-3.5 h-3.5 text-violet-500" />
        استورد من بنك الأسئلة
      </Button>
    </div>
  )
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={saving}
            className="gap-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white h-10"
          >
                ...
                >
                  ...
                  <div className="px-4 pt-3 space-y-2">
                    ...
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl border-2 border-dashed border-slate-300 text-slate-500 hover:border-blue-400 hover:text-blue-500 transition-all flex items-center justify-center gap-2 text-xs font-medium"
                    >
              ...
              <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                    {(["video", "text", "quiz", "pdf", "game", "assignment"] as const).map((type) => {
                    ...
                    <Input
                      ...
                      placeholder={
                        editingLesson.lesson.videoType === "youtube"
                          ...
                          : editingLesson.lesson.videoType === "url"
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 text-center hover:border-blue-400 hover:bg-blue-50/30 transition-all cursor-pointer">
                    <Paperclip className="w-6 h-6 text-slate-400 mx-auto mb-2" />
                    <p className="text-sm text-slate-500">
                      اسحب الملفات هنا أو اضغط للرفع
                    </p>
                    <p className="text-xs text-slate-400 mt-1">
                      PDF, ZIP, DOC, PPT — حد أقصى 50 ميجابايت
                    </p>
                  </div>
                </div>
          ...
          >
            <div className="p-5 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">بنك المحتوى</h3>
                  <p className="text-xs text-slate-500">اختر محتوى لإضافته إلى هذا القسم</p>
                </div>
              </div>
              <button onClick={() => { setShowContentBank(false); setBankTargetSectionId(null); setBankTargetSubsectionId(null) }} className="p-2 rounded-lg hover:bg-slate-100">
                <X className="w-5 h-5 text-slate-500" />
              </button>
            </div>
            <div className="p-5 overflow-y-auto flex-1">
              {(() => {
                const list = Array.isArray(contentBankData) ? contentBankData : (contentBankData as { data?: unknown[] })?.data
                const items = (list ?? []) as Array<{ id: string; title: string; titleAr?: string; type?: string; duration?: number; content?: string; videoUrl?: string; pdfUrl?: string }>
                if (items.length === 0) {
                  return (
                    <div className="text-center py-12">
                      <Library className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                      <p className="text-sm text-slate-500">لا يوجد محتوى في البنك بعد</p>
                      <p className="text-xs text-slate-400 mt-1">أضف محتوى من صفحة بنك المحتوى أولاً</p>
                    </div>
                  )
                }
                return (
                  <div className="grid gap-3">
                    {items.map((item) => (
                      <button
                        key={item.id}
                        onClick={() => addFromContentBank(item)}
                        className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-amber-300 hover:bg-amber-50/50 transition-all text-start group"
                      >
                        <div className="w-12 h-12 rounded-xl bg-slate-100 group-hover:bg-amber-100 flex items-center justify-center shrink-0">
                          {item.type === "VIDEO" ? <PlayCircle className="w-6 h-6 text-blue-500" /> : item.type === "PDF" ? <FileText className="w-6 h-6 text-red-500" /> : <FileText className="w-6 h-6 text-emerald-500" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-slate-900 truncate">{item.titleAr || item.title}</p>
                          <p className="text-xs text-slate-500">{item.type || "VIDEO"} • {item.duration || 0} دقيقة</p>
                        </div>
                        <Plus className="w-5 h-5 text-slate-400 group-hover:text-amber-600 shrink-0" />
                      </button>
                    ))}
                  </div>
                )
      ...
      {showQuestionBank && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <QuestionBankPickerModal
            ...
            onSelect={(selected) => addFromQuestionBank(selected)}
          />
        </div>
      )}
