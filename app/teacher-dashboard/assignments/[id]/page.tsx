...
export default function TeacherAssignmentGradingPage() {
  ...
  const { data: assignmentRes, loading, refetch } = useApi(() => api.getTeacherAssignment(assignmentId), { deps: [assignmentId], immediate: !!assignmentId })
  ...
  const handleGrade = async (s: Submission) => {
    ...
    setSaving(s.id)
    try {
      const res = await api.gradeTeacherAssignmentSubmission(assignmentId, s.studentId, { grade: gradeNum, feedback: draft.feedback || undefined })
      if (res.success) {
    ...
    <div className="space-y-6">
      <div>
        ...
        >
          ...
          {locale === "ar" ? "العودة للواجبات" : "Back to Assignments"}
        </Link>
        <h1 className="text-2xl font-bold text-[#0F172A]">{assignment.title}</h1>
        <p className="text-sm text-[#64748B] mt-1">
          {assignment.class?.name} — {submissions.length} {locale === "ar" ? "تسليم" : "submissions"}
        </p>
      </div>
