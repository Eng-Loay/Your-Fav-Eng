...
import { FileText, Search, Plus, Users, CheckCircle, X, Edit, Trash2, Loader2, AlertTriangle, ListPlus } from "lucide-react"
...
const defaultForm: ExamForm = { title: "", courseId: "", timeLimit: 30, maxAttempts: 2, passingScore: 60, showCorrectAnswer: true, randomizeQuestions: false, randomizeOptions: false }
...
export default function TeacherExamsPage() {
  ...
  const { data: examsData, loading, error, refetch } = useApi(() => api.getInstructorExams())
  const { data: resultsData, loading: resultsLoading } = useApi(() => api.getInstructorExamResults())
  ...
  const exams = useMemo(() => {
    ...
    return list.map((e: Record<string, unknown>) => ({
      ...
      courseTitle: (e.courseTitle as string) ?? ((e.course as Record<string, unknown>)?.titleAr as string) ?? ((e.course as Record<string, unknown>)?.title as string) ?? "",
      courseId: String(e.courseId ?? ""),
      questions: Number(e.questionsCount ?? e.questions ?? 0),
      timeLimit: Number(e.duration ?? e.timeLimit ?? 30),
      maxAttempts: Number(e.maxAttempts ?? 3),
      passingScore: Number(e.passingScore ?? 60),
      status: ((e.status as string) ?? "active") as "active" | "draft",
      participants: Number(e.participantsCount ?? e.participants ?? 0),
    }))
  }, [examsData])
  ...
  const results = useMemo(() => {
    ...
    return list.map((r: Record<string, unknown>) => ({
      id: String(r.id ?? ""),
      student: ((r.student as Record<string, unknown>)?.name as string) ?? (r.student as string) ?? "",
      exam: ((r.exam as Record<string, unknown>)?.title as string) ?? (r.exam as string) ?? "",
  ...
  const filtered = exams.filter((e) => e.title.toLowerCase().includes(search.toLowerCase()) || e.courseTitle.toLowerCase().includes(search.toLowerCase()))
  ...
  const openCreateModal = () => { setForm(defaultForm); setFormErrors({}); setEditingExam(null); setShowCreate(true) }
  const openEditModal = (exam: (typeof exams)[0]) => {
    setForm({ title: exam.title, courseId: exam.courseId, timeLimit: exam.timeLimit, maxAttempts: exam.maxAttempts, passingScore: exam.passingScore, showCorrectAnswer: true, randomizeQuestions: false, randomizeOptions: false })
    setFormErrors({}); setEditingExam(exam.id); setShowCreate(true)
  }
  const closeModal = () => { setShowCreate(false); setEditingExam(null); setForm(defaultForm); setFormErrors({}) }
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingExam) {
        await api.updateInstructorExam(editingExam, { title: form.title, courseId: form.courseId, duration: form.timeLimit, maxAttempts: form.maxAttempts, passingScore: form.passingScore, showCorrectAnswer: form.showCorrectAnswer, randomizeQuestions: form.randomizeQuestions, randomizeOptions: form.randomizeOptions })
        showToast(isAr ? "تم تحديث الاختبار" : "Exam updated", "success")
      } else {
        await api.createInstructorExam({ title: form.title, courseId: form.courseId, duration: form.timeLimit, maxAttempts: form.maxAttempts, passingScore: form.passingScore, showCorrectAnswer: form.showCorrectAnswer, randomizeQuestions: form.randomizeQuestions, randomizeOptions: form.randomizeOptions })
        showToast(isAr ? "تم إنشاء الاختبار" : "Exam created", "success")
      }
      closeModal(); refetch()
    } catch { showToast(isAr ? "حدث خطأ" : "Something went wrong", "error") }
    finally { setSaving(false) }
  }

  const handleDelete = async (id: string) => {
    setDeletingId(id); setShowDeleteConfirm(null)
    try {
      await api.deleteInstructorExam(id)
      showToast(isAr ? "تم حذف الاختبار" : "Exam deleted", "success"); refetch()
    } catch { showToast(isAr ? "فشل حذف الاختبار" : "Failed to delete exam", "error") }
  ...
  if (error) return <div className="flex flex-col items-center justify-center min-h-[400px] gap-4"><p className="text-red-500">{error}</p><Button onClick={refetch} variant="outline">{isAr ? "إعادة المحاولة" : "Retry"}</Button></div>
  ...
  return (
    <m.div variants={stagger} initial="initial" animate="animate" className="space-y-6">
      <m.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#0F172A]">{isAr ? "الاختبارات" : "Exams"}</h1>
          <p className="text-sm text-[#64748B] mt-1">{isAr ? "إنشاء وإدارة اختبارات دوراتك" : "Create and manage exams for your courses"}</p>
        </div>
        <Button onClick={openCreateModal} className="gap-2 rounded-xl bg-[#7C3AED] hover:bg-[#6D28D9] text-white">
      ...
      <m.div variants={fadeUp} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0]/60 shadow-sm">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#7C3AED]/10 text-[#7C3AED] mb-3"><FileText className="w-5 h-5" /></div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-[#E2E8F0]/60 shadow-sm">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#F59E0B]/10 text-[#F59E0B] mb-3"><Users className="w-5 h-5" /></div>
      ...
      <m.div variants={fadeUp}>
        <div className="flex gap-1 bg-[#F1F5F9] rounded-xl p-1 w-fit mb-4">
          <button onClick={() => setActiveTab("exams")} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === "exams" ? "bg-white text-[#0F172A] shadow-sm" : "text-[#64748B]"}`}>{isAr ? "الاختبارات" : "Exams"}</button>
          <button onClick={() => setActiveTab("results")} className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === "results" ? "bg-white text-[#0F172A] shadow-sm" : "text-[#64748B]"}`}>{isAr ? "النتائج" : "Results"}</button>
        </div>
      </m.div>
        ...
        <m.div variants={fadeUp} className="bg-white rounded-2xl border border-[#E2E8F0]/60 shadow-sm">
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              ...
              <Button onClick={openCreateModal} variant="outline" className="mt-2 rounded-xl gap-2 text-sm"><Plus className="w-4 h-4" /> {isAr ? "إنشاء اختبار" : "Create Exam"}</Button>
            </div>
          ) : (
                ...
                <tbody>
                  {filtered.map((exam) => (
                    <tr key={exam.id} className="border-t border-[#E2E8F0]/40 hover:bg-[#F8FAFC] transition-colors">
                      ...
                      <td className="px-3 py-3 text-sm text-[#64748B]">{exam.courseTitle}</td>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center gap-1">
                          <Link href={`/teacher-dashboard/exams/${exam.id}/questions`} className="p-1.5 rounded-lg hover:bg-[#7C3AED]/10 text-[#7C3AED]" title={isAr ? "إضافة أسئلة" : "Add questions"}>
                          ...
                          <button onClick={() => setShowDeleteConfirm(exam.id)} disabled={deletingId === exam.id} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 disabled:opacity-50">
      ...
      {activeTab === "results" && (
        <m.div variants={fadeUp} className="bg-white rounded-2xl border border-[#E2E8F0]/60 shadow-sm">
          {resultsLoading ? (
            <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 animate-spin text-[#7C3AED]" /></div>
          ) : results.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <FileText className="w-12 h-12 text-[#CBD5E1]" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0]/60 sticky top-0 bg-white rounded-t-2xl">
              <h3 className="text-lg font-bold text-[#0F172A]">{editingExam ? (isAr ? "تعديل الاختبار" : "Edit Exam") : (isAr ? "إنشاء اختبار جديد" : "Create New Exam")}</h3>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input value={form.title} onChange={(e) => { setForm((f) => ({ ...f, title: e.target.value })); setFormErrors((fe) => ({ ...fe, title: undefined })) }} className={`w-full border rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#7C3AED] ${formErrors.title ? "border-red-400" : "border-[#E2E8F0]/60"}`} placeholder={isAr ? "أدخل عنوان الاختبار" : "Enter exam title"} />
                {formErrors.title && <p className="text-xs text-red-500 mt-1">{formErrors.title}</p>}
              </div>
              <div>
                <label className="text-sm font-medium text-[#0F172A] block mb-1.5">{isAr ? "الدورة" : "Course"}</label>
                <select value={form.courseId} onChange={(e) => { setForm((f) => ({ ...f, courseId: e.target.value })); setFormErrors((fe) => ({ ...fe, courseId: undefined })) }} className={`w-full border rounded-xl px-3 py-2.5 text-sm outline-none bg-white ${formErrors.courseId ? "border-red-400" : "border-[#E2E8F0]/60"}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" value={form.timeLimit} onChange={(e) => setForm((f) => ({ ...f, timeLimit: Number(e.target.value) }))} min={1} className="w-full border border-[#E2E8F0]/60 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#7C3AED]" />
                </div>
                <div>
                  <label className="text-sm font-medium text-[#0F172A] block mb-1.5">{isAr ? "عدد المحاولات" : "Max Attempts"}</label>
                  <input type="number" value={form.maxAttempts} onChange={(e) => setForm((f) => ({ ...f, maxAttempts: Number(e.target.value) }))} min={1} className="w-full border border-[#E2E8F0]/60 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#7C3AED]" />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-[#0F172A] block mb-1.5">{isAr ? "درجة النجاح (%)" : "Passing Score (%)"}</label>
                <input type="number" value={form.passingScore} onChange={(e) => setForm((f) => ({ ...f, passingScore: Number(e.target.value) }))} min={0} max={100} className="w-full border border-[#E2E8F0]/60 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#7C3AED]" />
              </div>
              <div className="space-y-3 pt-2 border-t border-[#E2E8F0]/60">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.showCorrectAnswer} onChange={(e) => setForm((f) => ({ ...f, showCorrectAnswer: e.target.checked }))} className="rounded border-[#E2E8F0]" />
                  <span className="text-sm text-[#64748B]">{isAr ? "إظهار الإجابة الصحيحة بعد الاختبار" : "Show correct answer after quiz"}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.randomizeQuestions} onChange={(e) => setForm((f) => ({ ...f, randomizeQuestions: e.target.checked }))} className="rounded border-[#E2E8F0]" />
                  <span className="text-sm text-[#64748B]">{isAr ? "ترتيب الأسئلة عشوائياً" : "Randomize question order"}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={form.randomizeOptions} onChange={(e) => setForm((f) => ({ ...f, randomizeOptions: e.target.checked }))} className="rounded border-[#E2E8F0]" />
            ...
            <div className="p-5 border-t border-[#E2E8F0]/60 flex gap-3 justify-end">
              ...
              <Button onClick={handleSave} disabled={saving} className="rounded-xl bg-[#7C3AED] hover:bg-[#6D28D9] text-white">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            <div className="p-6 text-center">
              <div className="flex items-center justify-center w-12 h-12 rounded-full bg-red-100 mx-auto mb-4"><AlertTriangle className="w-6 h-6 text-red-600" /></div>
              <h3 className="text-lg font-bold text-[#0F172A] mb-2">{isAr ? "تأكيد الحذف" : "Confirm Delete"}</h3>
              <p className="text-sm text-[#64748B]">{isAr ? "هل أنت متأكد من حذف هذا الاختبار؟" : "Are you sure you want to delete this exam?"}</p>
            </div>
