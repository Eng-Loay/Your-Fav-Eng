...
export default function AdminCommunitiesPage() {
  ...
  const { data: communitiesRes, refetch: refetchCommunities } = useApi(() => api.getCommunities())
  const { data: usersRes } = useApi(() => api.getAdminUsers({ role: "STUDENT", page: 1 }), { deps: [] })

  const communities = (communitiesRes as { data?: any[] })?.data ?? []
  const usersData = usersRes as { data?: any[]; pagination?: any } | null
  ...
  const handleCreateCommunity = async () => {
    if (!newTitle.trim() || selectedStudents.length === 0) {
      showToast(locale === "ar" ? "أدخل العنوان واختر طلاباً" : "Enter title and select students", "error")
      return
    }
    try {
      const res = await api.createCommunity(newTitle.trim(), selectedStudents)
      if (res.success) {
        setShowCreate(false)
        setNewTitle("")
        setSelectedStudents([])
        refetchCommunities()
        showToast(locale === "ar" ? "تم إنشاء المجموعة" : "Community created", "success")
      }
    } catch {
      showToast(locale === "ar" ? "فشل إنشاء المجموعة" : "Failed to create community", "error")
    }
  }
  ...
  return (
    <div dir={dir} className="space-y-6">
      <m.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-[#0F172A]">
            {locale === "ar" ? "الكوميونتي" : "Communities"}
          </h1>
          <p className="text-sm text-[#94A3B8] mt-1">
            {locale === "ar" ? "أنشئ مجموعات واختر الطلاب للدردشة الجماعية" : "Create groups and select students for group chat"}
          </p>
        </div>
        <Button onClick={() => setShowCreate(true)} className="gap-2 rounded-xl bg-[#8B5CF6] hover:bg-[#7C3AED]">
          <Plus className="w-4 h-4" />
          {locale === "ar" ? "مجموعة جديدة" : "New Community"}
        </Button>
      </m.div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {communities.length === 0 && (
                <div className="p-8 text-center text-[#94A3B8] text-sm">
                  {locale === "ar" ? "لا توجد مجموعات. أنشئ مجموعة جديدة." : "No communities yet. Create a new one."}
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col">
            {!selectedCommunity ? (
              <div className="flex-1 flex items-center justify-center text-[#94A3B8] p-8">
                ...
                <p className="text-sm">{locale === "ar" ? "اختر مجموعة أو أنشئ واحدة" : "Select or create a community"}</p>
              </div>
            ) : (
              <>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="rounded-2xl max-w-md">
          <DialogHeader>
            <DialogTitle>{locale === "ar" ? "مجموعة جديدة" : "New Community"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label className="text-sm font-medium text-[#0F172A] block mb-1.5">{locale === "ar" ? "اسم المجموعة" : "Group Name"}</label>
              <input
                value={newTitle}
                onChange={(e) => setNewTitle(e.target.value)}
                placeholder={locale === "ar" ? "مثال: مجموعة الميكانيكا" : "e.g. Mechanics Group"}
                className="w-full border border-[#E2E8F0]/60 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#8B5CF6]"
              />
            </div>
            <div>
              <label className="text-sm font-medium text-[#0F172A] block mb-1.5">{locale === "ar" ? "اختر الطلاب" : "Select Students"}</label>
              <div className="max-h-48 overflow-y-auto space-y-2 border border-[#E2E8F0]/60 rounded-xl p-2">
                {students.length === 0 ? (
                  <p className="text-sm text-[#94A3B8] p-2">{locale === "ar" ? "لا يوجد طلاب" : "No students"}</p>
                ) : (
                  students.map((s: any) => (
                    <label
                      key={s.id}
                      className="flex items-center gap-3 p-2 rounded-lg hover:bg-[#F1F5F9] cursor-pointer"
                    >
                      <input
                        type="checkbox"
                        ...
                        className="rounded"
                      />
                      <div className="h-8 w-8 rounded-full overflow-hidden bg-[#E2E8F0] shrink-0">
                        {s.avatar ? (
                          <img src={resolveImageUrl(s.avatar)} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center text-[#94A3B8] font-bold text-sm">
                            {safeStr(s.name)[0]}
                          </div>
                        )}
                      </div>
            ...
            <Button onClick={handleCreateCommunity} className="w-full rounded-xl bg-[#8B5CF6] hover:bg-[#7C3AED]">
              {locale === "ar" ? "إنشاء المجموعة" : "Create Community"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
