...
export default function ExamQuestionsPage() {
  ...
  const { data: examData, loading, refetch } = useApi(() => api.getAdminExam(examId), { deps: [examId] })
  ...
  const addOption = () => {
    setNewQuestion((q) => ({
      ...q,
      options: [...q.options, { id: String.fromCharCode(97 + q.options.length), text: "", textAr: "", isCorrect: false }],
    }))
  }
  ...
  const updateOption = (idx: number, updates: Partial<{ text: string; textAr: string; isCorrect: boolean }>) => {
    setNewQuestion((q) => ({
      ...q,
      options: q.options.map((o, i) => (i === idx ? { ...o, ...updates } : updates.isCorrect ? { ...o, isCorrect: false } : o)),
    }))
  }
  ...
  const handleAddQuestion = async () => {
    if (!newQuestion.question.trim()) {
      showToast(isAr ? "السؤال مطلوب" : "Question is required", "error")
      return
    }
    ...
    try {
      ...
      await api.addAdminExamQuestion(examId, {
        question: newQuestion.question,
        questionAr: newQuestion.questionAr || undefined,
      ...
      showToast(isAr ? "فشل في إضافة السؤال" : "Failed to add question", "error")
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDeleteQuestion = async (qId: string) => {
    setDeletingId(qId)
    try {
      await api.deleteAdminExamQuestion(examId, qId)
      showToast(isAr ? "تم حذف السؤال" : "Question deleted", "success")
      refetch()
    } catch {
      showToast(isAr ? "فشل حذف السؤال" : "Failed to delete question", "error")
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const parseOptions = (opts: string | null | undefined) => {
    if (!opts) return []
    try {
      const arr = JSON.parse(opts) as Array<{ id?: string; text?: string; textAr?: string; isCorrect?: boolean }>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <Link
          href="/admin/exams"
          className="inline-flex items-center gap-2 text-sm text-[#64748B] hover:text-primary"
        >
      ...
      {questions.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs text-[#94A3B8] mt-1">{isAr ? "اضغط على إضافة سؤال لبدء إضافة الأسئلة" : "Click Add Question to start"}</p>
          <Button onClick={() => setShowAddModal(true)} className="mt-4 gap-2 rounded-xl">
            <Plus className="w-4 h-4" />
            {isAr ? "إضافة سؤال" : "Add Question"}
          </Button>
        </m.div>
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 disabled:opacity-50 flex-shrink-0"
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          ...
          >
            <div className="flex items-center justify-between p-5 border-b border-[#E2E8F0] sticky top-0 bg-white rounded-t-2xl">
              ...
              <button onClick={() => setShowAddModal(false)} className="flex items-center justify-center w-8 h-8 rounded-lg hover:bg-[#F1F5F9]">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewQuestion((q) => ({ ...q, questionAr: e.target.value }))}
                  className="w-full border border-[#E2E8F0] rounded-xl px-3 py-2.5 text-sm outline-none focus:border-primary"
                  placeholder={isAr ? "أدخل السؤال بالعربية" : "Enter question in Arabic"}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className="w-full border border-[#E2E8F0] rounded-xl px-3 py-2.5 text-sm outline-none focus:border-primary"
                  placeholder={isAr ? "أدخل السؤال بالإنجليزية" : "Enter question in English"}
                />
              </div>
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium text-[#0F172A]">{isAr ? "الخيارات" : "Options"}</label>
                  <Button type="button" variant="outline" size="sm" onClick={addOption} className="rounded-lg gap-1">
                  ...
                  {newQuestion.options.map((opt, idx) => (
                    <div key={opt.id} className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 border border-[#E2E8F0] rounded-lg px-2.5 py-1.5 text-sm outline-none focus:border-primary"
                        placeholder={isAr ? "النص عربي" : "Text (Ar)"}
                      />
                      <input
                        value={opt.text}
                        onChange={(e) => updateOption(idx, { text: e.target.value })}
                        className="flex-1 border border-[#E2E8F0] rounded-lg px-2.5 py-1.5 text-sm outline-none focus:border-primary"
                        placeholder={isAr ? "النص إنجليزي" : "Text (En)"}
                      />
            ...
            <div className="p-5 border-t border-[#E2E8F0] flex gap-3 justify-end">
              ...
              <Button onClick={handleAddQuestion} disabled={saving} className="rounded-xl bg-primary hover:bg-primary-hover text-white">
