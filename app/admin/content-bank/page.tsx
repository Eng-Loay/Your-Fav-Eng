...
export default function ContentBankPage() {
  ...
  const { data, loading, refetch } = useApi(() => api.getAdminContentBank())
  ...
  const handleAdd = async () => {
    ...
    setSaving(true)
    try {
      await api.createAdminContentBankItem({
        title: form.title,
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id)
    try {
      await api.deleteAdminContentBankItem(id)
      ...
      {items.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] overflow-hidden">
          <div className="divide-y divide-[#E2E8F0]">
            {items.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-4 hover:bg-[#F8FAFC]">
                <div>
                  <p className="text-sm font-medium text-[#0F172A]">{item.titleAr || item.title}</p>
