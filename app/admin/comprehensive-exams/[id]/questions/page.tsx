...
export default function ComprehensiveExamQuestionsPage() {
  ...
  const { data: examData, loading, refetch } = useApi(() => api.getAdminComprehensiveExam(examId), { deps: [examId] })
  ...
  const handleAddQuestion = async () => {
    ...
    if (newQuestion.type === "multiple_choice" || newQuestion.type === "true_false") {
      ...
      if (!newQuestion.correctAnswerText.trim()) {
    ...
    setSaving(true)
    try {
      await api.addAdminComprehensiveExamQuestion(examId, {
  ...
  const handleDelete = async (qId: string) => {
    setDeletingId(qId)
    try {
      await api.deleteAdminComprehensiveExamQuestion(examId, qId)
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Link href="/admin/comprehensive-exams" className="inline-flex items-center gap-2 text-sm text-[#64748B] hover:text-primary">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowAddModal(true)} className="gap-2 rounded-xl bg-primary">
                ...
                <div className="flex justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-semibold text-primary">{isAr ? `سؤال ${idx + 1}` : `Q${idx + 1}`}</span>
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <m.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-6">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border-2 text-xs font-medium transition-all ${
                        newQuestion.type === t.value ? "border-primary bg-primary/10 text-primary" : "border-[#E2E8F0] hover:border-[#94A3B8]"
                      }`}
                    >
              ...
              {newQuestion.type === "short_answer" ? (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewQuestion((q) => ({ ...q, correctAnswerText: e.target.value }))}
            ...
            <div className="flex gap-3 mt-6 justify-end">
              ...
              <Button onClick={handleAddQuestion} disabled={saving} className="rounded-xl bg-primary">
